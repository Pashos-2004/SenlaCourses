...
public class AutoServiceController {
    ...
    public void addGarageSpot(String number, int countOfPlacesForCars) {
        autoService.addGarageSpot(number,countOfPlacesForCars);
    }
    ...
    private void importGarageSpotFromMap(Map<String, String> row) {
    	try{
    		...
    		int countOfPlacesForCars = Integer.parseInt(row.get("countOfPlacesForCars"));
    		GarageSpot spot = autoService.findSpotById(id);
    		if(spot!=null) {
    			spot.setBox(box);
    			spot.setCountOfPlacesForCars(countOfPlacesForCars);
     			return;
    		}
    		autoService.addGarageSpot(box,countOfPlacesForCars);
    ...
    public void exportGarageSpotsToCSV(String filename) {
		String[] header = {"id","box","countOfPlacesForCars"};
		List<GarageSpot> spots = autoService.getGarageSpots();
		List<String[]> data = new ArrayList<>(spots.size());
		for(GarageSpot spot : spots) {
			data.add(new String[]{String.valueOf(spot.getId()),spot.getBox(),String.valueOf(spot.getCountOfPlacesForCars())});
		}
